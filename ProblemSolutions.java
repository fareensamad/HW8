...
class ProblemSolutions {
    ...
    public boolean canFinish(int numExams, int[][] prerequisites) {
        ...
        // Create directed graph's adjacency list
        ArrayList<Integer>[] adjacent = getAdjList(numExams, prerequisites);

        // Create an array to track in-degree of each node
        ...
        while (!queue.isEmpty()) {
            ...
            int node = queue.poll();
            // Increment count of processed nodes
            count++;

            // Decrease in-degree for all neighbors of the current node
        ...
        // If count of processed nodes is not equal to total nodes...
        if (count != numNodes) {
            // There is a cycle in the graph, return false
            return false;
        }

        return true; // All nodes processed, return true

    } // end of canFinish method
    ...
    private ArrayList<Integer>[] getAdjList(
        ...
        List<Integer>[] adj = (List<Integer>[]) new ArrayList[numNodes]; // Create an array of ArrayList ADT
    ...
    public int numGroups(int[][] adjMatrix) {
        ...
        int count = 0; // Count of groups

        // Loop through all nodes
        for (i = 0; i < numNodes; i++) {
            // If the node is not visited, it means a new group is found
            if (!visited[i]) {
                count++;
                dfs(graph, visited, i);
            }
